defmodule FaithfulWord.Actions.ReputationChangeConfigLoader do
  @moduledoc """
  ...
  def load(filename) do
    filename
    |> FaithfulWord.Utils.load_yaml_config()
    |> convert()
  end
  ...
  ## Examples
      ...
      iex> import FaithfulWord.Actions.ReputationChangeConfigLoader, only: [convert: 1]
  ...
  def convert(base_config) do
    Enum.reduce(base_config, %{}, fn {atom_action_type, value}, actions_map ->
      if FaithfulWord.DB.Type.UserActionType.valid_value?(atom_action_type) do
  ...
  defp convert_value(value) when is_map(value) do
    Enum.reduce(value, %{}, fn {entity, change_list}, action_changes ->
      if FaithfulWord.DB.Type.Entity.valid_value?(entity) do
