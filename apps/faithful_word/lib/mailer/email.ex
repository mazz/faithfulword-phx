defmodule FaithfulWord.Mailer.Email do
  @moduledoc """
  ...
  @sender_no_reply {"FaithfulWord", "no-reply@faithfulword.app"}
  ...
  def welcome(user) do
    ...
    user
    |> user_email()
    |> subject(gettext_mail_user(user, "Welcome to faithfulword.app!"))
  ...
  def reset_password_request(%ResetPasswordRequest{user: user, token: token, source_ip: ip}) do
    user
    |> user_email()
    |> subject(gettext_mail_user(user, "faithfulword.app - Reset your password"))
  ...
  def reputation_loss(user) do
    user
    |> user_email()
    |> subject(gettext_mail_user(user, "About your recent loss of reputation on FaithfulWord"))
  ...
  defp invitation_subject(nil),
    do: gettext_mail("Your invitation to try FaithfulWord.app is ready!")

  defp invitation_subject(user = %User{}),
    do:
      gettext_mail_user(
        user,
        "%{name} invited you to try FaithfulWord.app!",
  ...
  defp base_email() do
    ...
    |> put_html_layout({FaithfulWord.Mailer.View, "_layout.html"})
    |> put_text_layout({FaithfulWord.Mailer.View, "_layout.text"})
  end
